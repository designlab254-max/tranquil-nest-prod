...
async function startServer() {
  ...
  // Removed static file serving for /uploads; we'll handle this dynamically using the database
  console.log(`[SERVER] Uploads are now served from the database to persist across deployments.`);

  // Multer setup for image uploads (In-Memory to store in DB)
  ...
  // Image Upload Endpoint (Saves to Database)
  app.post('/api/upload', (req, res) => {
    upload.single('image')(req, res, async (err) => {
      ...
      if (!dbUrl) {
         return res.status(500).json({ error: 'Cannot persist uploads without a database connection.' });
      }
      ...
      try {
        ...
        console.log('[SERVER] Upload success (DB):', imageUrl);
        res.json({ imageUrl });
      } catch (dbErr) {
        console.error('[SERVER] Failed to save image to DB:', dbErr);
  ...
  app.get('/uploads/:filename', async (req, res) => {
    if (!dbUrl) {
       // Fallback to disk if necessary (shouldn't happen with valid dbUrl)
    ...
    try {
      const result = await pool.query('SELECT mime_type, data FROM uploaded_files WHERE filename = $1', [req.params.filename]);
      ...
      console.error('[SERVER] Failed to serve image:', err);
